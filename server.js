...
// const YTDLP_EXE  = process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp';
// const YTDLP_PATH = path.join(BIN_DIR, YTDLP_EXE);
// // FIX: Path to the securely ignored cookie file
...
//     '--no-playlist',
//     '--quiet', '--no-warnings',
//     // FIX: Add authentication cookies here to bypass age/bot checks
...
function buildGSIngestParts() {
  ...
  parts.push({
    text:
      'You are an expert YouTube strategist for long-form true-crime videos. ' +
      'First, read and internalize the following GOLD STANDARD reference data. ' +
      'Do NOT quote it back. Only use it to learn patterns.'
  });
...
app.post('/api/upload-video', upload.single('video'), async (req, res) => {
  ...
  //   if (!file || !file.uri) {
  //     throw new Error('Files API did not return a valid file URI');
  //   }

  //   update(80, 'Video uploaded. Preparing for generation…');

  //   return {
  //     fileUri: file.uri, // this is what fileData.fileUri expects
...
app.get('/api/history/:id/html', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(normalizeHtmlServer(data.html || ''));
  } catch (err) {
