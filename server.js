...
app.post('/api/upload-video', upload.single('video'), async (req, res) => {
  ...
  // const name     = req?.file?.originalname || 'uploaded-video.mp4';
  // server.js (line 699)
  // Get name first to determine MIME type from extension
  const name     = req?.file?.originalname || 'uploaded-video.mp4';
  
  // --- NEW LOGIC ---
  // Do NOT trust req.file.mimetype, as it's wrong on your VPS.
  // Instead, build the MIME type from the file extension.
  let mime;
  ...
  if (ext === '.mp4') mime = 'video/mp4';
    ...
    // As a fallback, try multer's mime *only* if it looks like a video
    const multerMime = req?.file?.mimetype;
    if (multerMime && multerMime.startsWith('video/')) {
      mime = multerMime;
    } else {
      mime = 'video/mp4'; // Final fallback
    }
  }
  // --- END NEW LOGIC ---
